...
export function FileAttachmentList({ attachments }: { attachments: FileAttachment[] }) {
  ...
  return (
    <div className="space-y-1.5">
      {attachments.map((a, i) => {
        const isPdf = a.mime_type === PDF_MIME;
        return (
          <a
            key={i}
            href={a.url}
            // PDFs open inline in the browser's built-in viewer (new tab).
            // Office files are force-downloaded via Content-Disposition: attachment
            // on the server; the download attribute also sets the save filename.
            {...(isPdf
          ...
          >
            ...
            <span className="flex-1 min-w-0">
              ...
              <span className="text-xs text-muted-foreground">
                {fmtSize(a.size)}{isPdf ? " · Opens in browser" : " · Click to download"}
              </span>
            </span>
            {isPdf
